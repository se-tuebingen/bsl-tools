...
function renderStepper(stepper: SI_STRUCT.Stepper): string {
    ...
    const str = `<stepper>
        ...
        ${stepperTree
            .map((el) =>
                renderStepResult(stepperTree, el as SI_STRUCT.StepResult)
            )
            .join("")}
        </div>
        <div class="buttons">
...
function renderStepResult(
    ...
    stepResult: SI_STRUCT.StepResult
): string {
    const currentStep = stepResult.currentStep;
    const programExpr = stepperTree
        .slice(0, currentStep)
        .map((stepResult) => stepResult.plugResult.expr); //renderExprs
...
function renderExpr(expr: BSL_AST.expr): string {
    if (BSL_AST.isCall(expr)) {
        ...
        const str = expr.options
            .map((el) => renderExpr(el.condition) + " -> " + renderExpr(el.result))
            .join("\n");
        return str;
    } else {
...
function renderSplitResult(splitResult: SI_STRUCT.SplitResult): string {
    if (SI_STRUCT.isSplit(splitResult)) {
        ...
        const contextStr = SI_STRUCT.isAppContext(context)
...
function renderPlugResult(plugResult: SI_STRUCT.PlugResult): string {
    ...
    <div class="plug">
        Plug Result: <pre><code>${SI_STRUCT.isValue(expr) ? renderValue(expr) : renderExpr(expr)
        }</code></pre>
    </div>`;
    return str;
}
...
function renderContext(context: SI_STRUCT.AppContext): string {
    ...
    const args = context.args
        .map((arg) => (SI_STRUCT.isValue(arg) ? renderValue(arg) : renderExpr(arg)))
        .join(" ");
    const str = `<pre><code>(${name} ${args})</code></pre>`;
    return str;
}
...
function previousStep(e: Event) {
    ...
    const wrapper = button.parentElement?.parentElement?.getElementsByClassName(
        "step-result-wrapper"
    )[0];
    const visibleResult = wrapper?.querySelector(
        ".step-result[visible=true]"
    ) as HTMLDivElement;
    const prevVisibleResult =
        visibleResult?.previousElementSibling as HTMLDivElement;
    // change Visibility
    visibleResult?.setAttribute("visible", "false");
    prevVisibleResult?.setAttribute("visible", "true");
    // show next button if hidden
    const nextButton = button.parentElement?.querySelector(
        "#nextButton"
...
function nextStep(e: Event): void {
    ...
    const prevButton = button.parentElement?.querySelector(
        "#prevButton"
    ) as HTMLButtonElement;
